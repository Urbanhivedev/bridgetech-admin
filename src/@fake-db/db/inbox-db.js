...
mock.onGet('/api/chat/get-chat').reply(request => {
	...
	return [
		200,
		{
			chat: inboxDB.chats.find(_chat => _chat.id === chatId),
...
mock.onGet('/api/chat/user').reply(config => {
	return [200, inboxDB.user[0]];
});

mock.onPost('/api/chat/user/data').reply(request => {
	const data = JSON.parse(request.data);
	inboxDB.user[0] = _.merge({}, inboxDB.user[0], data);
	return [200, inboxDB.user[0]];
});
...
mock.onPost('/api/chat/send-message').reply(request => {
	...
	const message = {
		who: inboxDB.user[0].id,
		message: messageText,
		time: new Date()
	};

	const chat = inboxDB.chats.find(_chat => _chat.id === chatId);
	chat.dialog = [...chat.dialog, message];
	inboxDB.user[0].chatList.find(_contact => _contact.contactId === contactId).lastMessageTime = message.time;

	return [
		200,
		{
			message,
			userChatList: inboxDB.user[0].chatList
		}
	];
});
