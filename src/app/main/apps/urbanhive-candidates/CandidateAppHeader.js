...
import { getProjects, selectProjects } from './store/candidatesSlice';
...
function CandidateAppHeader(props) {
  const { pageLayout } = props;
  const classes = useStyles(props);

  const dispatch = useDispatch();
  const widgets = useSelector(selectWidgets);
  const candidates = useSelector(selectProjects);
        ...
        <div className="flex items-center min-w-0">
          ...
          <div className="mx-12 min-w-0">
            ...
            <div className="flex items-center opacity-60 truncate">
              <Icon className="text-14 sm:text-24">account_tree</Icon>
              <Typography className="text-12 sm:text-14 font-medium mx-4 truncate">
                Build your network & Connect with experienced professionals.
              </Typography>
            </div>
          </div>
        </div>
